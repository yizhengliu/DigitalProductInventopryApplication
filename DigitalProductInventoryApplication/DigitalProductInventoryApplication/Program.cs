...
namespace DigitalProductInventoryApplication
{
    ...
    public interface IPrimiaryProperties 
    {
        int Id { get; set; }
        string Title { get; set; }
    }

    public abstract class ProductBase : IPrimiaryProperties
    {
    ...
    public abstract class CategoryBase : IPrimiaryProperties 
    {
        public int Id { get; set; }
    ...
    //abstract the construction of objects of varying types
    //class means T will be a reference, U means it will be derived from U class, new() means that it has a construct method that is parameterless
    public static class FactoryPattern<T, U> where T: class,U,new()
                                             where U: class,IPrimiaryProperties
    {
